<?php
namespace app\careserve\controller;
...
class ContentController extends SiteController {
    ...
    public function index()
    {
        ...
        //位置导航
        $crumb = target('lhave/Category')->loadCrumb($contentInfo['class_id']);
        //查询上级栏目信息
        $parentCategoryInfo = target('lhave/Category')->getInfo($categoryInfo['parent_id']);
        //获取顶级栏目信息
        $topCategoryInfo = target('lhave/Category')->getInfo($crumb[0]['class_id']);
        //更新访问计数
        $where = array();
        $where['content_id'] = $contentId;
        target('lhave/Content')->where($where)->setInc('views');
        //内容处理
        $contentInfo['content'] = html_out($contentInfo['content']);
        //扩展模型
        if($categoryInfo['fieldset_id']){
            $extInfo = target('lhave/FieldsetExpand')->getDataInfo($categoryInfo['fieldset_id'],$contentId);
        ...
        if(!empty($prevInfo)){
            $prevInfo['aurl']=target('lhave/Content')->getUrl($prevInfo,$appConfig);
            $prevInfo['curl']=target('lhave/Category')->getUrl($prevInfo,$appConfig);
        }
        //下一篇
        $nextWhere = array();
        ...
        if(!empty($nextInfo)){
            $nextInfo['aurl']=target('lhave/Content')->getUrl($nextInfo,$appConfig);
            $nextInfo['curl']=target('lhave/Category')->getUrl($nextInfo,$appConfig);
        }
        //MEDIA信息
