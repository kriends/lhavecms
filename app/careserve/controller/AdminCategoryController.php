...
class AdminCategoryController extends AdminController {
    ...
    protected function _infoModule(){
        $menu = target('lhave/AdminCategory','controller');
    ...
    public function add(){
        if(!IS_POST){
            $breadCrumb = array('栏目列表'=>url('lhave/AdminCategory/index'),'文章栏目添加'=>url());
    ...
    public function edit(){
        if(!IS_POST){
            ...
            $breadCrumb = array('栏目列表'=>url('lhave/AdminCategory/index'),'文章栏目修改'=>url());
            $this->assign('breadCrumb',$breadCrumb);
            $this->assign('name','修改');
            $this->assign('categoryList',target('lhave/Category')->loadList());
            $this->assign('tplList',target('admin/Config')->tplList());
    ...
    public function del(){
        ...
        //判断子栏目
        if(target('lhave/Category')->loadList(array(), $classId)){
            $this->error('请先删除子栏目！');
        }
        //判断栏目下内容
        $where = array();
        $where['A.class_id'] = $classId;
        $contentNum = target('ContentCar')->countList($where);
        if(!empty($contentNum)){
