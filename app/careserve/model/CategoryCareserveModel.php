<?php
namespace app\careserve\model;
use app\base\model\BaseModel;
/**
 * 栏目操作
 */
class CategoryCareserveModel extends BaseModel {
    //验证
    ...
    public function loadData($where = array(), $limit = 0 ,$order="A.sequence ASC , A.class_id ASC"){
        $pageList = $this->table("category as A")
                    ->join('{pre}category_careserve as B ON A.class_id = B.class_id')
                    ->field('B.*,A.*')
                    ->where($where)
                    ->limit($limit)
                    ->order($order)
    ...
    public function saveData($type = 'add'){
        ...
        $classId = target('lhave/Category')->saveData($type);
        if(!$classId){
            $this->error = target('lhave/Category')->getError();
            $this->rollBack();
            return false;
        }
        //分表处理
    ...
    {
        //总表
        target('lhave/Category')->delData($classId);
        //分表
        $map = array();
