...
class LabelService{
    ...
    public function contentList($data){
        ...
        if ($data['sub']&&!empty($data['class_id'])) {
            $classIds = target('lhave/Category')->getSubClassId($data['class_id']);
            $classWhere = '';
            if(!empty($classIds)){
        ...
        //其他属性
        $where['status'] = 1;
        return target('lhave/Content')->loadList($where,$data['limit'],$data['order']);
    }
    ...
    public function fragment($data){
        ...
        $info = target('lhave/Fragment')->getWhereInfo($where);
        if(empty($info)){
            return ;
        }
    ...
    public function formToken($data){
        ...
        $formInfo = target('lhave/FieldsetForm')->getWhereInfo($where);
        if(empty($formInfo)){
            return ;
        }
        return target('lhave/FieldsetForm')->setToken($data['table']);
    }
    ...
    public function aurl($data){
        ...
        $info = target('lhave/Content')->getWhereInfo($where);
        if(empty($info)){
            return ;
        }
        return target('lhave/Content')->getUrl($info);
    }
    ...
    public function curl($data){
        ...
        $info = target('lhave/Category')->getWhereInfo($where);
        if(empty($info)){
            return ;
        }
        return target('lhave/Category')->getUrl($info);
    }
    ...
    public function tagsList($data){
        ...
        if(!empty($data['name'])){
            ...
            foreach ($name as $key => $value) {
                ...
                $nameArray[$key]['url'] = url('lhave/TagsContent/index',array('name' =>$value));
                $nameArray[$key]['i'] = $key;
            }
            return $nameArray;
        }
        ...
        return target('lhave/Tags')->loadList($where, $data['limit'], $data['order']);
    }
    ...
    public function formList($data)
    {
        ...
        $fieldList = target('lhave/FieldForm')->loadList($where);
         //格式化表单内容为基本数据
        $data = array();
        ...
        if(!empty($list)){
            foreach ($list as $key => $value) {
                ...
                foreach ($fieldList as $v) {
                    $data[$key][$v['field']] = target('lhave/FieldData')->revertField($value[$v['field']],$v['type'],$v['config']);
                }                
            }
        }
        return $data;
