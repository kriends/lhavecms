...
class ContentModel extends BaseModel {
    ...
    public function delData($contentId){
        ...
        $fieldsetInfo = target('lhave/Fieldset')->getInfoClassId($info['class_id']);
        //删除扩展字段
        if(!empty($fieldsetInfo)){
            $expandModel = target('lhave/FieldData');
        ...
        //删除TAG关联
        target('lhave/TagsHas')->delData($map);
        return $status;
    }
    ...
    public function saveExtData($data){
        ...
        //获取字段集信息
        $fieldsetInfo = target('lhave/Fieldset')->getInfoClassId($classId);
        //保存扩展字段
        if(!empty($fieldsetInfo)){
            $expandModel = target('lhave/FieldData');
            //设置模型信息
    ...
    public function hasTags($keywords, $contentId){
        ...
        //设置模型
        $TagsHasModel = target('lhave/TagsHas');
        $TagsModel = target('lhave/Tags');
        //删除关联
        $where = array();
    ...
    public function getImage(){
        ...
        return target('lhave/ContentTools')->getImage($content, $getImageNum);
    }
