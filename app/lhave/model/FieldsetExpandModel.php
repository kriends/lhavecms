...
class FieldsetExpandModel extends BaseModel {
    ...
    public function getDataInfo($fieldsetId,$dataId)
    {
        ...
        //获取字段内容
        target('lhave/FieldData')->setTable('ext_'.$fieldsetInfo['table']);
        $extInfo = target('lhave/FieldData')->getInfo($dataId);
        if(empty($extInfo)){
            return ;
        }
        //获取字段列表
        ...
        $fieldList = target('lhave/FieldExpand')->loadList($where); 
        if(empty($fieldList)){
            return ;
        }
        $extArray = array();
        foreach ($fieldList as $value) {
            $extArray[$value['field']] = target('lhave/FieldData')->revertField($extInfo[$value['field']],$value['type'],$value['config']);
        }
        return $extArray;
