    ...
    public function index(){
        ...
        if(!empty($list)){
            ...
            foreach ($list as $key => $value) {
                $data[$key]=$value;
                $data[$key]['curl']=target('lhave/Category')->getUrl($value);
                $data[$key]['aurl']=target('lhave/Content')->getUrl($value);

            }
        }
        //位置导航
        $crumb = array(
            array('name'=>'标签列表','url'=>url('lhave/Tags/index')),
            array('name'=>$tagInfo['name'],'url'=>url('lhave/TagsContent/index',array('name'=>$tagInfo['name']))),
            );
        //URL参数
        $pageMaps = array();
        $pageMaps['name'] = $tag;
